...
from intelligence.DroneCityEnv_new import DroneCityEnv
...
# ---------------------------------------------------------------------------
"""
Problem : When you use Stable Baselines3 (SB3) to train an agent,
          it builds a standard PyTorch neural network by default.          
          By default, SB3 assumes your environment outputs one simple type of data.
          If your environment outputs a 1D array of numbers,
          SB3 uses a standard Multi-Layer Perceptron (MLP).
          If it outputs a 2D image, SB3 uses a Convolutional Neural Network (CNN).
          
solution : this function is written to override those defaults
           and tell SB3 exactly how to build a custom neural network
           that is tailored to this specific drone environment.
"""
...
def make_env_fn(api_port: int, seed: int):
    """
    Returns a callable that instantiates the environment.
    Required for SubprocessVecEnv parallelization.
    """
    def _init():
        env = DroneCityEnv(start=START_VOXEL, goal=GOAL_VOXEL, api_port=api_port)
        env = Monitor(env)
        env.reset(seed=seed)
        return env
    return _init
...
class DroneCustomExtractor(BaseFeaturesExtractor):
    ...
    def __init__(self, observation_space: spaces.Dict, features_dim: int = 256):
        ...
        # to hold a separate mini-network for each sensor.
        extractors = {}
        # how many data points will come out of all these sensors combined.
        total_concat_size = 0
...
def main(resume_path: Optional[str] = None) -> None:
    ...
    # The ports matching your launch_envs.py script
    train_ports = [41451, 41452] 
    ...
    # -- 3. Training environment (VECTORIZED) ---------------------------------
    logger.info(f"Spawning {len(train_ports)} parallel training environments...")
    env_fns = [make_env_fn(port, SEED + i) for i, port in enumerate(train_ports)]
    train_env = SubprocVecEnv(env_fns)

    # -- 4. Evaluation environment --------------------------------------------
    # WARNING: The Eval environment MUST have its own dedicated AirSim instance/port!
    # It cannot share ports with the training environments.
    eval_port = 41453 
    logger.info(f"Connecting Eval environment to port {eval_port}...")
    
    # Eval doesn't need to be vectorized, just standard Monitor wrapper
    eval_env_raw = DroneCityEnv(start=START_VOXEL, goal=GOAL_VOXEL, api_port=eval_port)
    eval_env = Monitor(eval_env_raw)
    eval_env.reset(seed=SEED + 99)
    ...
    # EvalCallback remains REMOVED to save hardware resources.
